...
public class PopupCadastrar extends PopupPanel {
	...
	public static PopupCadastrar getInstance(String endereco, LatLng ponto,
			MapWidget mapa) {
	...
	private VerticalPanel criaCorpoPopup() {
		...
		tipo.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				...
				if (tipo.getItemText(tipo.getSelectedIndex())
						.equals("Outro...")) {
	...
	private void salvarDados() {
		...
		if (outro.isVisible()) {
			marcador.setDados(localCrime.getText(), outro.getText(),
					descricao.getText(),
					hora.getText() + ":" + minuto.getText(), data.getValue());
		} else {
			marcador.setDados(localCrime.getText(),
					tipo.getItemText(tipo.getSelectedIndex()),
					descricao.getText(),
					hora.getText() + ":" + minuto.getText(), data.getValue());
		}
	...
	private boolean validaDados() {
		...
		if (outro.isVisible() && outroStr.equals("") || descricaoStr.equals("")
				|| horaStr.equals("") || !somenteNumeros(horaStr)
				|| !somenteNumeros(minutoStr) || data.getValue() == null) {
			...
			if (horaStr.equals("") || !somenteNumeros(horaStr)
					|| !somenteNumeros(minutoStr))
	...
	private boolean horarioValido(String hora, String minuto) {
		...
		if ((horaCrime >= 0 & horaCrime <= 23)
				&& (minutoCrime >= 0 & minutoCrime <= 59)) {
			return true;
		}
	...
	private void finalizaCadastro() {
		...
		greetingService.saveCrime(marcador.tipo,marcador.descricao,marcador.horario,marcador.data,lat,longi,teste);
	}
